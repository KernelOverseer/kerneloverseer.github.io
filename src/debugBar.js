class DebugBar extends Text {
  ...
  get fps() {
    let average = this.frameTimeHistory.reduce((prev, current) => {
      return prev + current;
  ...
  render(screen) {
    ...
    this.setContent(
      `QUEUE : ${this.screen.page.renderQueue.length} | SCREEN : ${this.screen.width} ${this.screen.height} | MOUSE : ${this.screen.mouse.x} ${this.screen.mouse.y} ${this.screen.mouse.down ? "DOWN" : "UP"} | FPS : ${this.fps}`
    );
